...
; copy all the sprite values to three 8 bit variables
; spr_x - x (9bit)
; spr_y - y
; spr_c - tile #
...
; 2 bits shifted in
; this is slow, so if this is zero
; skip it
; it was zeroed in oam_clear
...
; returns a16 = vram address offset (add it to the base address)
    ...
    txa
    and #$0020
    ora tilemap1            ; if either high bit is set, offset + $400
    beq @zero
    lda #$400
@zero:
    sta temp1
...
; returns collision = 1 or 0
    php
    A8
; 1) check if obj1 R (obj1 x + width) < obj2 L
...
; 2) check if obj1 L > obj2 R (obj2 x + width)
...
; 3) check if obj1 Bottom (obj1 y + height) < obj2 Top
...
; 4) check if obj1 Top > obj2 Bottom (obj2 y + height)
